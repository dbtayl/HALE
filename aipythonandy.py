...
def pythonAndyPlayTile(gs, playerNum):
	#FIXME: Return some oddball value we can recognize when debugging
	print("In pythonAndyPlayTile! FINISH ME!")
	return 17

#Return a chain_t (integer 0 <= n < NUM_CHAINS)
#Should be a valid chain to form
def pythonAndyFormChain(gs, playerNum):
	#FIXME: Return some oddball value we can recognize when debugging
	print("In pythonAndyFormChain! FINISH ME!")
	return 31

#Returns a chain_t (integer 0 <= n < NUM_CHAINS) from the array of options
def pythonAndyMergerSurvivor(gs, playerNum, options):
	#FIXME: Return some oddball value we can recognize when debugging
	print("In pythonAndyMergerSurvivor! FINISH ME!")
	return 5

#Returns an array of order to merge chains in. See player.h.
#Return array size should be NUM_CHAINS
def pythonAndyMergerOrder(gs, playerNum, survivor, options):
	#FIXME: Return some oddball value we can recognize when debugging
	print("In pythonAndyMergerOrder! FINISH ME!")
	return [0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF]

#Return list of length NUM_CHAINS, indicating which stocks to buy
def pythonAndyBuyStock(gs, playerNum):
	#FIXME: Return some oddball value we can recognize when debugging
	print("In pythonAndyBuyStock! FINISH ME!")
	return [0, 1, 2, 4, 8, 16, 32]

#Return (tradefor, sell) tuple, one number each
def pythonAndyMergerTrade(gs, playerNum, survivor, defunct):
	#FIXME: Return some oddball value we can recognize when debugging
	print("In pythonAndyMergerTrade! FINISH ME!")
	return (2, 3)

#Return essentially true/false, or 0/<non-zero>
def pythonAndyEndGame(gs, playerNum):
	#FIXME: Return some oddball value we can recognize when debugging
	print("In pythonAndyEndGame! FINISH ME!")
	return 101
