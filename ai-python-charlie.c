...
static uint8_t initPython()
{
	...
	pModName = PyUnicode_DecodeFSDefault(AI_PYTHON_CHARLIE_MOD_NAME);
	...
	pyFuncPlayTile = PyObject_GetAttrString(pModule, AI_PYTHON_CHARLIE_FUNC_PLAY_TILE);
	pyFuncFormChain = PyObject_GetAttrString(pModule, AI_PYTHON_CHARLIE_FUNC_FORM_CHAIN);
	pyFuncMergerSurvivor = PyObject_GetAttrString(pModule, AI_PYTHON_CHARLIE_FUNC_MERGER_SURVIVOR);
	pyFuncMergerOrder = PyObject_GetAttrString(pModule, AI_PYTHON_CHARLIE_FUNC_MERGER_ORDER);
	pyFuncBuyStock = PyObject_GetAttrString(pModule, AI_PYTHON_CHARLIE_FUNC_BUY_STOCK);
	pyFuncMergerTrade = PyObject_GetAttrString(pModule, AI_PYTHON_CHARLIE_FUNC_MERGER_TRADE);
	pyFuncEndGame = PyObject_GetAttrString(pModule, AI_PYTHON_CHARLIE_FUNC_END_GAME);
...
const PlayerActions_t pythonCharlieActions = {	.playTile = pythonCharliePlayTile,
					.formChain = pythonCharlieFormChain,
					.mergerSurvivor = pythonCharlieMergerSurvivor,
					.mergerOrder = pythonCharlieMergerOrder,
					.buyStock = pythonCharlieBuyStock,
					.mergerTrade = pythonCharlieMergerTrade,
					.endGame = pythonCharlieEndGame,
					.typeName = "aipythoncharlie"
				};
