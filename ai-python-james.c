...
static uint8_t initPython()
{
	...
	pModName = PyUnicode_DecodeFSDefault(AI_PYTHON_JAMES_MOD_NAME);
	...
	pyFuncPlayTile = PyObject_GetAttrString(pModule, AI_PYTHON_JAMES_FUNC_PLAY_TILE);
	pyFuncFormChain = PyObject_GetAttrString(pModule, AI_PYTHON_JAMES_FUNC_FORM_CHAIN);
	pyFuncMergerSurvivor = PyObject_GetAttrString(pModule, AI_PYTHON_JAMES_FUNC_MERGER_SURVIVOR);
	pyFuncMergerOrder = PyObject_GetAttrString(pModule, AI_PYTHON_JAMES_FUNC_MERGER_ORDER);
	pyFuncBuyStock = PyObject_GetAttrString(pModule, AI_PYTHON_JAMES_FUNC_BUY_STOCK);
	pyFuncMergerTrade = PyObject_GetAttrString(pModule, AI_PYTHON_JAMES_FUNC_MERGER_TRADE);
	pyFuncEndGame = PyObject_GetAttrString(pModule, AI_PYTHON_JAMES_FUNC_END_GAME);
...
const PlayerActions_t pythonJamesActions = {	.playTile = pythonJamesPlayTile,
					.formChain = pythonJamesFormChain,
					.mergerSurvivor = pythonJamesMergerSurvivor,
					.mergerOrder = pythonJamesMergerOrder,
					.buyStock = pythonJamesBuyStock,
					.mergerTrade = pythonJamesMergerTrade,
					.endGame = pythonJamesEndGame,
					.typeName = "aipythonjames"
				};
