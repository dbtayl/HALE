...
def pythonSamplePlayTile(gs, playerNum):
	#FIXME: Return some oddball value we can recognize when debugging
	print("In pythonSamplePlayTile! FINISH ME!")
	return 17

#Return a chain_t (integer 0 <= n < NUM_CHAINS)
#Should be a valid chain to form
def pythonSampleFormChain(gs, playerNum):
	#FIXME: Return some oddball value we can recognize when debugging
	print("In pythonSampleFormChain! FINISH ME!")
	return 31

#Returns a chain_t (integer 0 <= n < NUM_CHAINS) from the array of options
def pythonSampleMergerSurvivor(gs, playerNum, options):
	#FIXME: Return some oddball value we can recognize when debugging
	print("In pythonSampleMergerSurvivor! FINISH ME!")
	return 5

#Returns an array of order to merge chains in. See player.h.
#Return array size should be NUM_CHAINS
def pythonSampleMergerOrder(gs, playerNum, survivor, options):
	#FIXME: Return some oddball value we can recognize when debugging
	print("In pythonSampleMergerOrder! FINISH ME!")
	return [0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF]

#Return list of length NUM_CHAINS, indicating which stocks to buy
def pythonSampleBuyStock(gs, playerNum):
	#FIXME: Return some oddball value we can recognize when debugging
	print("In pythonSampleBuyStock! FINISH ME!")
	return [0, 1, 2, 4, 8, 16, 32]

#Return (tradefor, sell) tuple, one number each
def pythonSampleMergerTrade(gs, playerNum, survivor, defunct):
	#FIXME: Return some oddball value we can recognize when debugging
	print("In pythonSampleMergerTrade! FINISH ME!")
	return (2, 3)

#Return essentially true/false, or 0/<non-zero>
def pythonSampleEndGame(gs, playerNum):
	#FIXME: Return some oddball value we can recognize when debugging
	print("In pythonSampleEndGame! FINISH ME!")
	return 101
